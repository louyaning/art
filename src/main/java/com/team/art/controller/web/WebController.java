...
import com.team.art.service.course.impl.CourseServiceImpl;
	...
	public String index(CourseWare courseWare, HttpServletRequest request ){
		...
		try{
			if(courses.size()>0){
				...
				//坑爹操作一
				courseWare.setCourseId(courceService.listCoursesByPid(courses.get(0).getId()).get(0).getId());
				List<CourseWare> listCoursesWareByBranch = courseWareService.listCoursesWareByBranch(courseWare);	
